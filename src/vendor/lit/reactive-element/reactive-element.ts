...
// Global declarations for webpack compatibility
...
(globalObject as any).litPropertyMetadata = (globalObject as any).litPropertyMetadata || new WeakMap();
(Symbol as any).metadata = Symbol.metadata || Symbol('metadata');
...
if (NODE_MODE) {
  globalObject.customElements ??= customElements;
}
...
const polyfillSupport = DEV_MODE
  ? globalObject.reactiveElementPolyfillSupportDevMode
  : globalObject.reactiveElementPolyfillSupport;

if (DEV_MODE) {
  // Ensure warnings are issued only 1x, even if multiple versions of Lit
  // are loaded.
  globalObject.litIssuedWarnings ??= new Set();
  ...
  issueWarning = (code: string, warning: string) => {
    ...
    if (
      !globalObject.litIssuedWarnings!.has(warning) &&
      !globalObject.litIssuedWarnings!.has(code)
    ) {
      console.warn(warning);
      globalObject.litIssuedWarnings!.add(warning);
    }
  };
  ...
  queueMicrotask(() => {
    ...
    if (globalObject.ShadyDOM?.inUse && polyfillSupport === undefined) {
      issueWarning(
...
(globalObject as any).litPropertyMetadata ??= new WeakMap<
  object,
  Map<PropertyKey, PropertyDeclaration>
>();
...
export abstract class ReactiveElement
  ...
  // shadowed by this import, so it no longer refers to the globalObject.
  extends HTMLElement
  implements ReactiveControllerHost
{
  ...
  protected static finalize() {
    ...
    if (metadata !== null) {
      const properties = (globalObject as any).litPropertyMetadata.get(metadata);
...
(globalObject.reactiveElementVersions ??= []).push('2.1.0');
if (DEV_MODE && globalObject.reactiveElementVersions.length > 1) {
  queueMicrotask(() => {
    issueWarning!(
      'multiple-versions',
